...
export default withAuth(async function handler(req, res) {
  ...
  try {
    const { githubLink, itchLink } = req.body;
    ...
    const bossFought = (
      await base("jungleBossesFought").create([
        {
          fields: {
            ID: stretchId,
            githubLink,
    ...
    const jungleStretchesRecords = await base("jungleStretches")
      .select({
        filterByFormula: `
            AND(
    ...
    for (let i = 0; i < jungleStretchesRecords.length; i++) {
      ...
      const currentBossesFought =
        jungleStretchesRecord.fields.jungleBossesFought || [];
