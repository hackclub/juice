...
export default withAuth(async function handler(req, res) {
  ...
  try {
    ...
    const activeRecords = await base("juiceStretches")
      .select({
        filterByFormula: `AND(
        NOT({endTime} != ''),
        NOT({isCanceled} = TRUE()),
    ...
    if (activeRecords && activeRecords.length > 0) {
      activeRecords.forEach((record, index) => {
        ...
        if (fields.startTime) {
          ...
          const workTimeSeconds = Math.max(
            0,
            durationSeconds - pauseTimeSeconds,
          );
          ...
          console.log(
            `  Time worked: ${workTimeHours.toFixed(2)} hours (${workTimeSeconds.toFixed(0)} seconds)`,
          );
          console.log(`  Start time: ${startTime.toISOString()}`);
          console.log(`  Last active: ${fields.pauseTimeStart}`);
          if (fields.totalPauseTimeSeconds) {
            console.log(
              `  Total pause time: ${fields.totalPauseTimeSeconds} seconds`,
            );
          }
          console.log("---");
    ...
    console.log(
      `Total active time worked: ${totalTimeWorkedHours.toFixed(2)} hours (${totalTimeWorkedSeconds.toFixed(0)} seconds)`,
    );
