...
export default withAuth(async function handler(req, res) {
  ...
  try {
    ...
    const records = await base("jungleStretches")
      .select({
    ...
    const timeToReward =
      Math.floor(Math.random() * (600000 - 300000 + 1)) + 300000; // Randomly generated between 300k and 600k ms -> 5m and 10m (average 7.5m)
    ...
    if (lastCollectedFruitTime) {
      if (
        new Date().getTime() >
        new Date(lastCollectedFruitTime).getTime() + timeToReward
      ) {
        const probabilityRecords = await base(
          "fruitPricesProbabilities - Do not modify",
        )
          .select({})
          .firstPage();
        const probabilities = probabilityRecords.map(
          (record) => record.fields.probability,
        );
        ...
        if (selectedFruit == "kiwis") {
          const oldKiwisCollected =
            records[0].fields.kiwisCollected == undefined
              ? 0
              : records[0].fields.kiwisCollected;
          ...
          const oldLemonsCollected =
            records[0].fields.lemonsCollected == undefined
              ? 0
              : records[0].fields.lemonsCollected;
          await base("jungleStretches").update([
            {
              id: records[0].id,
              fields: {
          ...
          const oldOrangesCollected =
            records[0].fields.orangesCollected == undefined
              ? 0
              : records[0].fields.orangesCollected;
          await base("jungleStretches").update([
            {
              id: records[0].id,
              fields: {
          ...
          const oldApplesCollected =
            records[0].fields.applesCollected == undefined
              ? 0
              : records[0].fields.applesCollected;
          await base("jungleStretches").update([
            {
              id: records[0].id,
              fields: {
          ...
          const oldBlueberriesCollected =
            records[0].fields.blueberriesCollected == undefined
              ? 0
              : records[0].fields.blueberriesCollected;
          await base("jungleStretches").update([
            {
              id: records[0].id,
              fields: {
