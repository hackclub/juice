...
import MainViewZero from '@/components/screens/MainViewZero';
...
export default function Zero() {
  ...
  useEffect(() => {
    ...
    if (authToken) {
      ...
      setStage('loading'); // Start with 3D world if not logged in
    }
  }, []);
  ...
  return (
    <>
      <Head>
        ...
        <link rel="icon" href="/favicon-zero.ico" />
      </Head>
      {!isMobile ? (
        <main className={styles.main}>
          ...
          {stage === 'computer' && (
            <MainViewZero
              ...
              windowsOpen={[
                ...(searchParams.get('windows')
                  ? searchParams.get('windows').split('-')
                  : ['juiceWindow']),
                userData ? '' : 'register',
              ]}
            />
