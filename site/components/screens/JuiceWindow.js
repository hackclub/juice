...
export default function JuiceWindow({
  position,
  isDragging,
  isActive,
  handleMouseDown,
  ...
  const nonWhisperIdleAudioFiles = ["./nonWhisperIdle1.mp3"];
  ...
  const playExp = () => {
    if (expSoundRef.current) {
      ...
      expSoundRef.current
        .play()
        .catch((e) => console.error("Error playing exp:", e));
    }
  };
  ...
  useEffect(() => {
    ...
    idleWhisperAudioRefs.current = idleWhisperAudioFiles.map((file) => {
      ...
      audio.volume = 0.5;
      audio.onerror = (e) =>
        console.error("Error loading idle whisper audio:", e);
      return audio;
    });
    nonWhisperIdleAudioRefs.current = nonWhisperIdleAudioFiles.map((file) => {
      ...
      audio.volume = 0.5;
      audio.onerror = (e) =>
        console.error("Error loading non-whisper idle audio:", e);
      return audio;
    });
  }, []);
  ...
  const playRandomWhisper = () => {
    const randomIndex = Math.floor(
      Math.random() * whisperAudioRefs.current.length,
    );
    ...
    selectedAudio
      ...
      .catch((e) =>
        console.error(
          "Error playing whisper audio (whisper to yourself for now :P) :",
          e,
        ),
      );
  };
  ...
  const playRandomIdleWhisper = () => {
    if (whisperCount < 3) {
      const randomIndex = Math.floor(
        Math.random() * idleWhisperAudioRefs.current.length,
      );
      const selectedAudio = idleWhisperAudioRefs.current[randomIndex];
      selectedAudio.currentTime = 0;
      selectedAudio
        .play()
        .catch((e) => console.error("Error playing idle whisper audio:", e));
      whisperCount++;
    } else {
      const randomIndex = Math.floor(
        Math.random() * nonWhisperIdleAudioRefs.current.length,
      );
      ...
      selectedAudio
        .play()
        .catch((e) =>
          console.error("Error playing non-whisper idle audio:", e),
        );
      whisperCount = 0;
    }
  };
    ...
    if (isJuicingLocal && startTime && !stopTime && !isPaused) {
      interval = setInterval(() => {
        ...
        const diff = Math.floor(
          (now - startTime) / 1000 - totalPauseTimeSeconds,
        );
  ...
  useEffect(() => {
    ...
    if (
      (!isJuicingLocal && isWhisperEnabled) ||
      (isPaused && isWhisperEnabled)
    ) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const now = new Date();
        const diff = Math.floor(
          (now - startTimeDate) / 1000 - data.totalPauseTimeSeconds,
        );
        const minutes = Math.floor(diff / 60);
        const seconds = diff % 60;
        setTimeJuiced(`${minutes}:${seconds.toString().padStart(2, "0")}`);
  ...
  const handleStartJuicing = async () => {
    if (
      !confirm(
        "Just to confirm, you have your game editor ready and you're ready to start working on your game?",
      )
    ) {
      return;
    }
  ...
  const handleEndStretch = async () => {
    ...
    try {
      ...
      alert(
        "Failed to create OMG moment. Please try again with a smaller file or refresh  the page",
      );
    } finally {
  ...
  const handleCancelStretch = async () => {
    if (
      !confirm(
        "Are you sure you want to cancel this juice stretch? Your time won't be logged.",
      )
    ) {
      return;
    }
        ...
        >
          {!showExplanation ? (
            <>
              ...
              {isJuicing && (
                <p>
                  Log your time working on a feature then share "OMG IT WORKS"
                  moment when you make it work
                </p>
              )}
              ...
              {isJuicingLocal && (
                ...
                >
                  ...
                  >
                    {isSubmitting
                      ? "Juicing..."
                      : 'End Stretch with your "OMG IT WORKS" moment'}
                  </button>
                  ...
                  >
                    {isWhisperEnabled
                      ? "Disable Whisper Audio"
                      : "Enable Whisper Audio"}
                  </button>
                </div>
              )}
            </>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <p>
                Juicer is a way to gamify your process making mini-ships for
                your game & to log the time you spend making them. When you
                start working on your game, open the Juicer and "Start Juicing".
                Once you have an "OMG IT WORKS MOMENT" capture that beautiful
                moment & share it with the Juice community. We'll come and give
                kudos to congratulate you & you'll get credit for that time :){" "}
                <br />
                <br />
                The Juicer is how you will log your time to hit the 100 hour
                game achievement.
              </p>
              <button
                onClick={() => {
