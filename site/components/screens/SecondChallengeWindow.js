...
export default function SecondChallengeWindow({
  position,
  isDragging,
  isActive,
  handleMouseDown,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("/api/submit-second-challenge", {
        ...
        body: JSON.stringify({ itchLink, platforms: selectedPlatforms }),
      });
  ...
  const hasCompleted =
    userData?.achievements?.includes("second_challenge_completed") || isSuccess;
    ...
    >
      ...
      >
        ...
        >
          <p>Hey,</p>
          <p>
            Your challenge this week is to make a proof of concept for the core
            mechanic of your game. Your game doesn't have to look beautiful,
            have any assets, or even a start/stop menu. All you need is a little
            bit of playable content for the core mechanic of your game.
          </p>
          <p>
            Once you have that core game play, you can ship it to{" "}
            <a
              href="https://itch.io/developers"
              target="_blank"
              rel="noopener noreferrer"
            >
              itch.io
            </a>{" "}
            & then your game will be reviewed & if it works you'll get the 2nd
            achievement. Deploy to as many platforms as you'd like
          </p>
          ...
          {isInReview && (
            <p style={{ color: "blue", margin: 0 }}>
              Itch.io link submitted! I'll check out your project & let you know
              if it works. Keep juicing ~Thomas{" "}
          ...
          {hasCompleted && (
            <p style={{ color: "green", margin: 0 }}>
              Challenge completed! Achievement unlocked: Second Challenge
              Complete 🎉
            </p>
          )}
        </div>
